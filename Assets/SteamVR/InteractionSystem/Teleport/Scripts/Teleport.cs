...
namespace Valve.VR.InteractionSystem
{
	...
	public class Teleport : MonoBehaviour
    {
		...
		//public Transform destinationFormationTransform;//HMB
		//public Transform mirrorPlayerTransform;//HMB
		public Transform mirrorDestinationFormationTransform;//HMB
		...
		void Awake()
        {
			...
			mirrorDestinationFormationTransform = GameObject.Find("MirrorDestinationFormation").transform;//HMB
		}
		...
		*/
		...
		private void HidePointer()
		{
			...
			destinationReticleTransform.gameObject.SetActive( false );
			//destinationFormationTransform.gameObject.SetActive(false);//HMB
			//mirrorPlayerTransform.gameObject.GetComponent<MirrorPalyer>().CmdSetPreAvatarsActive(false);//HMB
			mirrorDestinationFormationTransform.gameObject.GetComponent<MirrorDestinationFormation>().SetPreAvatarsActive(false);//HMB
			invalidReticleTransform.gameObject.SetActive( false );
			offsetReticleTransform.gameObject.SetActive( false );
		...
		private void TryTeleportPlayer()
		{
			if ( visible && !teleporting )
			{
				if ( pointedAtTeleportMarker != null && pointedAtTeleportMarker.locked == false )
				{
					...
					CancelTeleportHint();
					//--------------------HMB----------------------
					//destinationFormationTransform.gameObject.GetComponent<DestinationFormation>().TakeFollowers();
					//mirrorPlayerTransform.gameObject.GetComponent<MirrorPalyer>().CmdTakeFollowers();
					mirrorDestinationFormationTransform.gameObject.GetComponent<MirrorDestinationFormation>().TakeFollowers();
					//--------------------HMB----------------------
				}
			}
		}
		...
		private void TeleportPlayer()
		{
			...
			//--------------------HMB----------------------
			//transform.Find("DestinationFormation").gameObject.SetActive(true);
			//teleportPosition = destinationFormationTransform.gameObject.GetComponent<DestinationFormation>().GetGuideTransform().position;
			//teleportPosition = mirrorPlayerTransform.gameObject.GetComponent<MirrorPalyer>().GetPreGuideTransform().position;
			//teleportPosition = mirrorDestinationFormationTransform.gameObject.GetComponent<MirrorDestinationFormation>().GetGuideTransform().position;
			teleportPosition = mirrorDestinationFormationTransform.gameObject.GetComponent<MirrorDestinationFormation>().GetGuidePosition();
			//Debug.Log("teleportPosition：" + teleportPosition);
			//transform.Find("DestinationFormation").gameObject.SetActive(false);
			//--------------------HMB----------------------
            ...
            {
                Vector3 playerFeetOffset = player.trackingOriginTransform.position - player.feetPositionGuess;
                player.trackingOriginTransform.position = teleportPosition + playerFeetOffset;//这句话导致跳转！
				//Debug.Log("player.trackingOriginTransform.position：" + player.trackingOriginTransform.position);
				//--------------------------HMB-------------------------------
				//mirrorDestinationFormationTransform.gameObject.GetComponent<MirrorDestinationFormation>().GetGuidePositionAfterTeleporting();
				player.transform.eulerAngles = mirrorDestinationFormationTransform.gameObject.GetComponent<MirrorDestinationFormation>().GetGuideEulerAngles();
				//mirrorDestinationFormationTransform.gameObject.GetComponent<MirrorDestinationFormation>().RecordScore();
				//--------------------------HMB-------------------------------
		...
		private void UpdatePointer()
		{
			...
			if (hitTeleportMarker != null) //Hit a teleport marker
			{
...
#endif
					destinationReticleTransform.gameObject.SetActive(hitTeleportMarker.showReticle);
					//destinationFormationTransform.gameObject.SetActive(hitTeleportMarker.showReticle);//HMB??
					//mirrorPlayerTransform.gameObject.GetComponent<MirrorPalyer>().CmdSetPreAvatarsActive(hitTeleportMarker.showReticle);//HMB
					mirrorDestinationFormationTransform.gameObject.GetComponent<MirrorDestinationFormation>().SetPreAvatarsActive(hitTeleportMarker.showReticle);//HMB
				}
				...
				if (showPlayAreaMarker)
				{
					//Show the play area marker if this is a teleport area
					TeleportArea teleportArea = pointedAtTeleportMarker as TeleportArea;
					if (teleportArea != null && !teleportArea.locked && playAreaPreviewTransform != null)// playAreaPreviewTransform == null
					{
						Vector3 offsetToUse = playerFeetOffset;
						//Adjust the actual offset to prevent the play area marker from moving too much
						if (!movedFeetFarEnough)
			...
			//取消显示目标十字
			destinationReticleTransform.gameObject.SetActive(false);//HMB
			destinationReticleTransform.position = pointedAtPosition;
			//destinationFormationTransform.position = pointedAtPosition;
			//mirrorPlayerTransform.gameObject.GetComponent<MirrorPalyer>().CmdSetDestinationPosition(pointedAtPosition);
			mirrorDestinationFormationTransform.position = pointedAtPosition;//HMB
